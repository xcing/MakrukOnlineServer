...
class ServiceController extends Controller {
    ...
    public $userIdTest = '1';
    public $ads = false;
    public $ip_server = '103.13.229.209';
    ...
    public function actionGetUser() {
        ...
        $model = UserFront::model()->findByPk($userId);
    ...
    public function CheckForceUpdate($data) {
        if ($data[1] != $this->signatureKey) {
            die('fail');
        }
        $version = $data[2];
    ...
    public function FindMatch($data) {
        ...
        $sql = "UPDATE `match` SET userId2 = " . $statuserModel->statuserId . " WHERE userId2 = 0 AND userId1 != 0 AND userId1 != " . $statuserModel->statuserId . " AND scoreMatch1 between " . ($statuserModel->score - 200) . " AND " . ($statuserModel->score + 200) . " AND createdDate > NOW() - INTERVAL 11 SECOND LIMIT 1";
    ...
    public function Register($data) {
        ...
        $model = UserFront::model()->findByAttributes(array('email' => $email));
        if ($model == null) {
            $model = new UserFront();
            $model->email = $email;
    ...
    public function RegisterAfterConnectDevice($data) {
        ...
        if ($model == null) {
            if ($guestId == null) {
                $model = UserFront::model()->findByAttributes(array('facebookId' => $facebookId));
            } else {
                $model = UserFront::model()->findByAttributes(array('guestId' => $guestId));
            }
            $model->email = $email;
            $model->password = md5($password);
            if ($model->save()) {
        ...
        if ($model != null) {
            if (md5($password) == $model->password) {
                if ($model->active == 1) {
                    ...
                    $datas['xsignature'] = Encryption::encryptRJ256($this->ip_server);
    ...
    public function SetDisplayName($data) {
        ...
        $model = UserFront::model()->findByAttributes(array('displayName' => $displayName));
        if ($model != null) {
            echo "already";
        } else {
            $model = UserFront::model()->findByPk($userId);
    ...
    public function actionSetdisplayname() {
        $userId = Yii::app()->request->getParam('userId');
        $displayName = Yii::app()->request->getParam('displayName');
