    ...
    if (e.ctrlKey || e.metaKey) {
        ...
        if (!files.length)
            ...
            if (files.length > 1)
                ...
                var fullurl = 'http://' + window.location.host + window.location.pathname;
                var path = '/images/upload/' + browser.dir + '/' + data.name;
                //var path = url.substring(0, url.length - 20) + '/images/upload/' + browser.dir + '/' + data.name;
                //var fullpath = fullurl.substring(0, fullurl.length - 20) + '/images/upload/' + browser.dir + '/' + data.name
                $('#fileinfo').html(data.name + ' (' + this.humanSize(data.size) + ', ' + data.date + ') , path = ' + path);
                //$('#fileinfo').html(data.name + ' (' + this.humanSize(data.size) + ', ' + data.date + ') , path = ' + path + ' , fullpath = ' + fullpath);
            }
        }
    } else {
        var data = file.data();
